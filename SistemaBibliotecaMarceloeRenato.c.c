		...
		switch(opMenuInicial){//inicio controle opções inicias
			case 1:
				...
				switch(opLogin){//inicio validação de usuário
					...
					case 3:
						do{//inicio loop menu Funcionario
							printf(":~:~:~:~:~|Opções de Funcinário|~:~:~:~:~:\n\n"
								   "[1] - Cadastrar Aluno\n"
							...
							switch(opMenuADM){//inicio controle opções funcionario
								case 1:
									CadastrarAln();
								break;
								...
								default:
									Error01();
							}//fim controle opções funcionario
						}while(opMenuADM != 0);//fim loop menu Funcionario
					break;
...
int Login(){//inicio login
	int op = 0, i;//Variáveis de controle de sub-rotinas
	...
	for(i=0; i<5; i++){
		if((strcmp(auxLogin, Func[i].Id.login) == 0) && (strcmp(auxSenha, Func[i].Id.senha) == 0)){//define log de func
			op = 3;
		}
	}
				...
				do{
					...
					for(i=0; i<5; i++){// verifica se op de livro existe e, se sim, empresta o respectivo
						if(Liv[i].nPosicao == auxEmp){
							...
							strcpy(Aln[p].LivEmp[iEmps[p]], Liv[i].nome);
							iEmps[p]++;
							Aln[p].QtdEmp++;
							Liv[i].nAln = p;
							idVal = 1;
...
void DevolucaoLiv(){//inicio devolução
	int i, p, auxDev, n;
	...
	if(LivsEmp > 0){
		...
		for(i=0; i<5; i++){//verifica se aluno existe
		...
		if(!val){
			...
			do{
				...
				for(i=0; i<5; i++){//lista livros emprestados pelo aluno
					for(n=0; n<3; n++){
						if(strcmp(Aln[p].LivEmp[n], "NULL") == 1){
							if((strcmp(Aln[p].LivEmp[n], Liv[i].nome) == 0) && (strcmp(Liv[i].status, "emprestado") == 0)){
								printf("[%i] - %s\n", e, Aln[p].LivEmp[n]);
								Liv[i].nPosicao = e;
								e++;
							}
				...
				for(i=0; i<5; i++){// verifica se op de livro foi emprestado, se sim, devolve o respectivo
					for(n=0; n<3; n++){
						if((Liv[i].nPosicao == auxDev) && (strcmp(Aln[p].LivEmp[n], Liv[i].nome) == 0)){
...
void Relatorio(){//inicio relatório
	int i;
	int LivsEmp = 0;
	...
	if(LivsEmp > 0){
		...
		for(i=0; i<5; i++){//imprimi dados de livros emprestados
			if(strcmp(Liv[i].status, "emprestado") == 0){
				printf("Emprestado para: %s\n"
					   ...
					   "Área: %s\n\n",
					   Aln[Liv[i].nAln].nome,
					   Aln[Liv[i].nAln].matricula,
					   Aln[Liv[i].nAln].curso,
					   Liv[i].nome,
					   Liv[i].isbn,
					   Liv[i].Curso.nome,
					   Liv[i].Curso.area);
